...
import com.keenworks.example.akka.avro.evolution.actor.Fact.{FactHistory, GetFactHistory, RegisterFact}
...
class Fact extends PersistentActor with ActorLogging {
  ...
  override def receiveCommand: Receive = {
    case RegisterFact(statement) =>
      ...
      persist(Statement(statement, TruthMatrix(5.2, 4.5), Some(true)))(updateFactHistory)
    case GetFactHistory =>
